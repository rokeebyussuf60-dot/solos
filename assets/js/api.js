(function(){
  ...
  function uuid(){
    ...
    const arr = new Uint8Array(16); if(window.crypto && crypto.getRandomValues) crypto.getRandomValues(arr); else for(let i=0;i<16;i++) arr[i]=Math.floor(Math.random()*256);
  ...
  function withTimeout(promise, ms){
    ...
    return {signal:ctl.signal, done: promise.finally(()=>clearTimeout(timer))};
  }
  API.call = async function(action, data={}, opts={}){
    ...
    if(method === 'GET'){
      const qs = new URLSearchParams(payload).toString(); url += (url.includes('?') ? '&' : '?') + qs;
    } else {
    ...
    try{
      ...
      let out; const text = await res.text();
      try{ out = JSON.parse(text); } catch { throw new Error('Invalid response from server.'); }
      if(res.status === 401 || out.code === 'UNAUTHORIZED') { API.clearSession(); window.dispatchEvent(new CustomEvent('solos:session-expired')); }
      if(!res.ok || out.success === false){ const err = new Error(out.message || 'Unable to complete request.'); err.response = out; throw err; }
      return out;
    } catch(err){
